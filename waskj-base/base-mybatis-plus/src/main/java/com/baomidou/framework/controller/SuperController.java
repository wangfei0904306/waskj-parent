...
public abstract class SuperController {
	...
	protected abstract String getPageIndexParamName();
	protected abstract String getPageSizeParamName();
	...
	protected <C> Page<C> getPage(Class<C> clazz){
		int _size = 10, _index = 1;
		if ( request.getParameter(getPageSizeParamName()) != null ) {
			_size = Integer.parseInt(request.getParameter(getPageSizeParamName()));
		}
		if ( request.getParameter(getPageIndexParamName()) != null ) {
			_index = Integer.parseInt(request.getParameter(getPageIndexParamName()));
		}
		return new Page<C>(_index,_size);
	}

	protected <C> Page<C> getPage(Class<C> clazz,int size){
		int _index = 1;
		if ( request.getParameter(getPageIndexParamName()) != null ) {
			_index = Integer.parseInt(request.getParameter(getPageIndexParamName()));
		}
		return new Page<C>(_index,size);
	}
