...
public class QueryFilter {
	...
	public String[] getRightJoinFetch() {
		return rightJoinFetch;
	}

	public void setRightJoinFetch(String[] rightJoinFetch) {
		this.rightJoinFetch = rightJoinFetch;
	}
	...
	public QueryFilter(HttpServletRequest request) {
		...
		while (names.hasMoreElements()) {
			...
			if (StringUtils.isNotBlank(value)) {
				...
				if (name.equalsIgnoreCase("rightJoinFetch")) {
					this.setRightJoinFetch(value.replaceAll(" ", "").split(","));
				}
				if (name.equalsIgnoreCase("rightJoin")) {
					this.setRightJoin(value.replaceAll(" ", "").split(","));
				}
			}
		}
	}
		...
		if (name.toUpperCase().indexOf("Q_") > -1) {
			...
			if (commands.length == 3 || commands.length == 4) {
				...
				if ((commands[2].equalsIgnoreCase("=") || commands[2].equalsIgnoreCase("EQ")) && commands.length == 4) {// 等号操作符
					...
					if (commands[3].equalsIgnoreCase("BigDecimal")) {
